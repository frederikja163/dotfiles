-- Yazi's stock video previewer (yazi-plugin/preset/plugins/video.lua), with
-- hardware decoding switched off.
--
-- Why this exists: the stock previewer hands ffmpeg `-hwaccel auto`. On this
-- machine that resolves to the Radeon VCN video engine, and VCN does not
-- survive the way yazi uses it -- a short decode per keyframe, opened and torn
-- down as the cursor moves through a directory. The kernel logs mmhub page
-- faults from the VCN rings, the decoder ring times out, and amdgpu performs
-- a full mode1 reset that reports "VRAM is lost". Every GL client loses its
-- buffers in the same instant: Hyprland aborts inside CHyprOpenGLImpl::begin()
-- (a SIGABRT, from an assert), xdg-desktop-portal-hyprland segfaults, and
-- hyprpaper, hypridle, kitty and Discord all come down with it. The session
-- then restarts and repeats for as long as a video is under the cursor.
--
-- Software decoding costs CPU, but never touches VCN, which is the whole
-- point. It is also what upstream would do without the flag: `-hwaccel none`
-- is ffmpeg's default and is written out here to make the choice explicit and
-- to stop a future upstream edit restoring it silently.
--
-- Only one line differs from upstream, marked below. Everything else is a
-- verbatim copy so this keeps behaving exactly like the previewer it replaces
-- (same cache file, same skip/percent logic, same spot panel). Re-sync it if
-- yazi changes video.lua.
...
function M:preload(job)
	...
	local cmd = Command("ffmpeg")
		...
		:arg { "-v", "warning", "-hwaccel", "none", "-threads", 1, "-an", "-sn", "-dn" } -- was "-hwaccel", "auto"
